 ...
 * Merges functionality from errorHandler.js and ErrorManager.js
 * Provides unified error processing and user-friendly messaging
 */
...
class ConsolidatedErrorHandler {
  ...
  translateError(error, context = {}) {
    ...
    if (this.config.enableRetryQueue && this.canRetry(errorInfo) && context.retryable !== false) {
      this.addToRetryQueue(error, context, errorInfo);
    }
  ...
  isUserFriendlyMessage(message) {
    const technicalTerms = [
      'stack trace', 'null pointer', 'undefined', 'internal error',
      'database', 'sql', 'mongodb', 'redis', 'jwt', 'cors'
    ];
  ...
  getErrorSeverity(errorInfo) {
    ...
    if (type === 'network') return 'warning';
   ...
   */
  addToRetryQueue(error, context, errorInfo) {
    const retryKey = `${context.method || 'GET'}_${context.url}_${Date.now()}`;
  ...
  async processRetryQueue() {
    ...
    for (const [key, item] of this.retryQueue.entries()) {
      if (item.nextRetry <= now && item.attempts < this.config.maxRetryAttempts) {
        toRetry.push({ key, item });
      } else if (item.attempts >= this.config.maxRetryAttempts) {
    ...
    for (const { key, item } of toRetry) {
      try {
        // Attempt retry (this would need to be implemented based on the original request)
        item.attempts++;
...
const consolidatedErrorHandler = new ConsolidatedErrorHandler();

export default consolidatedErrorHandler;
export { ConsolidatedErrorHandler };